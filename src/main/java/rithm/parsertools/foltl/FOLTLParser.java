...
import rithm.parsertools.foltl.foltlgrammarParser.PredContext;
import rithm.parsertools.foltl.foltlgrammarParser.QuantContext;
class FOLTLErrorListener extends BaseErrorListener
{
	Boolean validation_result;
	String error_message;
	public FOLTLErrorListener()
	{
		super();
...
public class FOLTLParser extends ParserPlugin{
	/**
	...
	private HashMap<String, ArrayList<String>> objectIdsForSpec;
	/**
	...
	private foltlgrammarLexer lexer;
	private foltlgrammarParser parser;
	...
	public FOLTLParser(String Lang)
	{
		SpecLang = Lang;
		...
		objectIdsForSpec = new HashMap<>();
	}
	class RiTHMfoltlgrammarListener extends foltlgrammarBaseListener
	{
		int mode;
		String spec;
		public RiTHMfoltlgrammarListener(int mode, String spec)
		{
			super();
			this.mode = mode;
			this.spec = spec;
...
//			System.out.println(PredCount.get(ctx.PREDNAME().toString()));
			switch (mode) {
			case 1:
				predString = ctx.PREDNAME().toString().replaceAll("\\([A-Z]+\\)", "");
				RitHMPredicate rp = new DefaultRiTHMPredicate(predString);
				...
				if(!predsForSpec.get(spec).contains(predString))
					predsForSpec.get(spec).add(predString);
				break;
			case 2:
				predString = ctx.PREDNAME().toString().replaceAll("\\([A-Z]+\\)", "");
				RitHMPredicate rp1 = new DefaultRiTHMPredicate(predString);
...
//					System.out.println("deleted from PredicateList" + ctx.PREDNAME().toString());
				...
				if(predsForSpec.get(spec).contains(predString))
					predsForSpec.get(spec).remove(predString);
				break;
			default:
				break;
			}
		...
		public void enterQuant(QuantContext ctx) {
			// TODO Auto-generated method stub
			if(!objectIdsForSpec.containsKey(spec))
				objectIdsForSpec.put(spec, new ArrayList<>());
			objectIdsForSpec.get(spec).add(ctx.OBJNAME().toString());
		}
	...
	{
		lexer = new foltlgrammarLexer(new ANTLRInputStream(Spec.getTextDescription()));
		lexer.removeErrorListeners();
		FOLTLErrorListener le2 = new FOLTLErrorListener();
		lexer.addErrorListener(le2);
		CommonTokenStream tokenstream = new CommonTokenStream(lexer);
		RiTHMfoltlgrammarListener rllistener = new RiTHMfoltlgrammarListener(mode, Spec.getTextDescription());
		parser = new foltlgrammarParser(tokenstream);
		ParseTreeWalker ptwalker = new ParseTreeWalker();
		parser.removeErrorListeners();
		FOLTLErrorListener le1 = new FOLTLErrorListener();
		parser.addErrorListener(le1);
		ParseTree ptree = parser.foltl();
		ptwalker.walk(rllistener, ptree);
		boolean retval = le1.validation_result && le2.validation_result;
		ptwalker = null;
		
		if(retval)
			errorMessages.put(Spec, "No error, Valid FOLTL formula\n");
		else
	...
	public String rewriteSpec(RitHMSpecification spec) {
		// removes quantifiers and return
		String rewrittenSpec = spec.getTextDescription();
		rewrittenSpec = rewrittenSpec.replaceAll("FORALL\\([A-Z]+\\)", "");
		rewrittenSpec = rewrittenSpec.replaceAll("EXISTS\\([A-Z]+\\)", "");
		rewrittenSpec = rewrittenSpec.replaceAll("\\([A-Z]+\\)", "");
//		System.out.println(rewrittenSpec);
	...
	public ArrayList<String> getObjectIDs(String spec) {
		// TODO Auto-generated method stub
		return objectIdsForSpec.get(spec);
	}
