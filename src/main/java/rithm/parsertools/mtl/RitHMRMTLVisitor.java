...
public class RitHMRMTLVisitor extends MTLParserBaseVisitor<String>{
	 ...
	 * 
	 */
	final static Logger logger = Logger.getLogger(RitHMRMTLVisitor.class);
	/**
	 * @param progStateCollection
	 */
	public RitHMRMTLVisitor(RitHMProgStateCollection progStateCollection)
	{
		super();
...
//				System.out.println(rtVal);
	...
	private RitHMTruthValue createTval(String tvalue, double timestamp){
		return new DefaultRiTHMTimedTruthValue(tvalue,timestamp);
	}
	/**
	 * @param ctxBase
	 * @param previousName1
			...
			if(optimized){
				...
				for(int i = 0; i < operandEval2.size();i++){
					if(operandEval2.get(i).getTruthValueDescription().equals("true")){
						m = 0;
						rtVal= createTval("true",operandEval1.get(i).getTimetamp());
						binaryTemporalEval.add(rtVal);
						continue;
					}
					
					if(operandEval1.get(i).getTruthValueDescription().equals("false")){
						m = 0;
						rtVal= createTval("false",operandEval1.get(i).getTimetamp());
						binaryTemporalEval.add(rtVal);
						continue;
					}
					if(i > 0){
						if(endInterval - operandEval2.get(i).getTimetamp() + operandEval2.get(i-1).getTimetamp()
								>= m){
							rtVal= createTval("true",operandEval1.get(i).getTimetamp());
							binaryTemporalEval.add(rtVal);
						}else{
					...
					m = m + operandEval2.get(i).getTimetamp();
				}
			...
			if(optimized){
				...
				for(int i = 0; i < operandEval.size();i++){
					if(operandEval.get(i).getTruthValueDescription().equals("true")){
						m = 0;
						rtVal= createTval("true",operandEval.get(i).getTimetamp());
						unaryTemporalEval.add(rtVal);
						continue;
					}
					if(i > 0){
						if(endInterval - operandEval.get(i).getTimetamp() + operandEval.get(i-1).getTimetamp()
								>= m){
							rtVal= createTval("true",operandEval.get(i).getTimetamp());
							unaryTemporalEval.add(rtVal);
						}else{
							rtVal= createTval("false",operandEval.get(i).getTimetamp());
							unaryTemporalEval.add(rtVal);
						}
					}else{
						m = 0;
					...
					m = m + operandEval.get(i).getTimetamp();
				}
